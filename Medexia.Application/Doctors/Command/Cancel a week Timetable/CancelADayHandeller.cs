...
{
    public class CancelADayHandeller : IRequestHandler<CancelADayCommand, Resultt>
    {
        private readonly IHttpContextAccessor acc;
        private readonly ITimeTable timeRepo;
        private readonly IDoctor docRepo;

        public CancelADayHandeller(IHttpContextAccessor acc, ITimeTable TimeRepo, IDoctor docRepo)
        {
            this.acc = acc;
            timeRepo = TimeRepo;
            this.docRepo = docRepo;
        }
        public async Task<Resultt> Handle(CancelADayCommand request, CancellationToken cancellationToken)
        {
            ...
            if (TimeTables.IsActive == true)
            {
                TimeTables.IsActive = false;
                timeRepo.UpdateAsync(TimeTables);
                timeRepo.saveAsync();
                return new Resultt { IsSuccess = true, Message = $"Timetable {TimeTables.Id} has Been Suspended Till Start Again" };
            }
            return new Resultt { IsSuccess = false, Message = $"TimeTable {TimeTables.Id} Is Already Suspended" };
